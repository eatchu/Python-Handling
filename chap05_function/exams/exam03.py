...
def bank_account(bal):
    ...
    def getBalance():  # 잔액확인(getter)
        print('현재 계좌 잔액은 {}원 입니다.'.format(balance))
        return balance

    def deposit(balance):  # 입금하기(setter)
        money = int(input('입금액을 입력하세요 : '))
        balance += money
        print('{}원 입금후 잔액은 {}원 입니다.'.format(money, balance))
        return balance

    def withdraw(balance):  # 출금하기(setter)
        money = int(input('출금액을 입력하세요 : '))
        if balance >= money:
            balance -= money
            print('{}원 출금후 잔액은 {}원 입니다.'.format(money, balance))
            return balance
        else:
            print('잔액이 부족합니다')
